...
class AntWorld {
    ...
    get_pixel_coords(pixel_id) {
        return [
            mod(pixel_id, this.width),
            Math.floor(pixel_id / this.width)
        ];
    } 
    ...
    set_trail_int(x, y, v) {
        [x, y] = this.bound_coords(x, y);
        if (Math.abs(v) < this.min_trail) {
            v = 0.0;
        }
        this.trails[y][x] = v;
        if (Math.abs(v) >= this.min_trail) {
            this.active_pixel_ids.add(this.get_pixel_id(x, y));
        } else {
            this.active_pixel_ids.delete(this.get_pixel_id(x, y));
        }
    }
    ...
    update() {
        ...
        for (let ant of this.ants) {
            ...
            ant.x += 1 * Math.cos(ant.a);
            ant.y += 1 * Math.sin(ant.a);
            [ant.x, ant.y] = this.bound_coords(ant.x, ant.y);

            // if (Math.random() < 0.001) {
            //     //this.add_ant(ant.x, ant.y, Math.random() * 2 * Math.PI);
            //     ant.a += Math.PI / 2;
            // }
        }

        // only run evaporation on active pixels
        for (const active_id of this.active_pixel_ids) {
            const [x, y] = this.get_pixel_coords(active_id);
            this.set_trail_int(x, y, this.get_trail_int(x, y) * this.evaporation);
        }
    }
    ...
    draw() {
        ...
        ctx.fillText(this.active_pixel_ids.size, 0, 0);
    }
}

const world = new AntWorld(1000, 1000, ctx);
for (i = 0; i < 6000; i++) {
    world.add_ant(250, 250, Math.random() * 2 * Math.PI);
}
...
distance_slider.oninput = function() {
    ...
    world.sight_distance = v;
    distance_text.innerHTML = v;
};
...
function animate() {
    ...
    if (mouse_down) {
        // place trails 20 times in a line from last mouse position to current
        // mouse position, offsetting each trail randomly by a small amount
        for (let i = 0; i < 20; i++) {
            const r = Math.random() * 1.5;
            const t = Math.random() * 2 * Math.PI;
            world.add_trail(
                last_mouse_x + i / 19 * (mouse_x - last_mouse_x) + r * Math.cos(t), 
                last_mouse_y + i / 19 * (mouse_y - last_mouse_y) + r * Math.sin(t)
            );
        }
    }
    world.draw();
